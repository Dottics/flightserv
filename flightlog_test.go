...
func TestService_CreateFlightLog(t *testing.T) {
	...
	for i, tc := range tt {
		...
		t.Run(name, func(t *testing.T) {
			...
			if tc.exchange.Request.Method != "POST" {
				t.Errorf("expected method POST got %s", tc.exchange.Request.Method)
			}
			if !dutil.ErrorEqual(tc.e, e) {
				t.Errorf("expected error %v got %v", tc.e, e)
			}
...
func TestService_UpdateFlightLog(t *testing.T) {
	tt := []struct {
		name     string
		exchange *microtest.Exchange
		log      FlightLog
	...
	for i, tc := range tt {
		...
		t.Run(name, func(t *testing.T) {
			...
			log, e := s.UpdateFlightLog(tc.log)

			if tc.exchange.Request.Method != "PUT" {
				t.Errorf("expected method PUT got %s", tc.exchange.Request.Method)
			}
			if !dutil.ErrorEqual(tc.e, e) {
				t.Errorf("expected error %v got %v", tc.e, e)
			}
